...
uartinit(void)
{
  ...
  lcr_cache = ReadReg(LCR);
  WriteReg(LCR, (LCR_BAUD_LATCH | lcr_cache));
  WriteReg(0, (unsigned char)(divisor & 0xff));
  WriteReg(1, (unsigned char)((divisor >> 8) & 0xff));


  /* restore the DLAB to access the baud rate divisor registers */
  WriteReg(LCR, lcr_cache);

  /* 8 data bits, 1 stop bit, no parity, clear DLAB */
  WriteReg(LCR, (0x03 | 0x00 | 0));

  WriteReg(MDC, 0); /*disable flow control*/
...
#define FCR_FIFO_8 0X80

   WriteReg(FCR, (FCR_FIFO_ENABLE|FCR_FIFO_CLEAR | FCR_MODE1 | /*FCR_FIFO_1*/FCR_FIFO_8 ));

   WriteReg(IER, IER_TX_ENABLE | IER_RX_ENABLE);  //endable the interrupt
...
{
  do{
  }while((serial_in(LSR) & LSR_TX_IDLE) == 0);
  serial_out(THR, c);
  return 0;
}
