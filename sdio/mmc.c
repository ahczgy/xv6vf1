...
int mmc_get_op_cond(struct mmc *mmc)
{
	...
	if (err) {
		 ...
		 */
		printf("Unable to do a full power cycle. Disabling the UHS modes for safety\n");
		uhs_en = false;
...
static void print_mmcinfo(struct mmc *mmc)
{
	...
	if (!IS_SD(mmc) && mmc->version >= MMC_VERSION_4_41) {
		...
		printf("HC WP Group Size: ");
		printf(((u64)mmc->hc_wp_grp_size) << 9, "\n");

		printf("User Capacity: ");
		printf(mmc->capacity_user, usr_enh ? " ENH" : "");
		if (mmc->wr_rel_set & EXT_CSD_WR_DATA_REL_USR)
			printf(" WRREL\n");
		else
			printf('\n');
		if (usr_enh) {
			printf("User Enhanced Start: ");
			printf(mmc->enh_user_start, "\r\n");
			printf("User Enhanced Size: ");
			printf(mmc->enh_user_size, "\r\n");
		}
		printf("Boot Capacity: ");
		printf(mmc->capacity_boot, has_enh ? " ENH\r\n" : "\r\n");
		printf("RPMB Capacity: ");
		printf(mmc->capacity_rpmb, has_enh ? " ENH\r\n" : "\r\n");
